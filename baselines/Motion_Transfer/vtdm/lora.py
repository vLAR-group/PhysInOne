...
def fuse_lora_all_linear(model, target_replace_module, lora_scale=1.0, safe_fusing=True):
    ...
    for _module, name, _child_module in _find_modules(
        ...
        # define module
        regular_linear_layer = _child_module.linear
        dtype, device = regular_linear_layer.weight.data.dtype, regular_linear_layer.weight.data.device

        # start merge params---only weight
        w_orig = regular_linear_layer.weight.data.float()
        w_up = _child_module.lora_up.weight.data.float()
        w_down = _child_module.lora_down.weight.data.float()
        fused_weight = w_orig + (lora_scale * torch.bmm(w_up[None, :], w_down[None, :])[0])

        if safe_fusing and torch.isnan(fused_weight).any().item():
            raise ValueError(
                "This LoRA weight seems to be broken. "
                f"Encountered NaN values when trying to fuse LoRA weights for {model}."
                "LoRA weights will not be fused."
            )

        regular_linear_layer.weight.data = fused_weight.to(device=device, dtype=dtype)

        # back to model structure
        _module._modules[name] = regular_linear_layer

        _child_module.lora_up.weight.requires_grad = False
        _child_module.lora_down.weight.requires_grad = False
        _child_module = _child_module.cpu()
