...
OPENCLIP_H_REVISION = "1c2b8495b28150b8a4922ee1c8edee224c284c0c"
...
class FrozenOpenCLIPImageEmbedder(AbstractEmbModel):
    ...
    def __init__(
        ...
        super().__init__()
        pretrained = self._resolve_local_open_clip_checkpoint(arch, version)
        model, _, _ = open_clip.create_model_and_transforms(
            arch,
            device=torch.device(default(init_device, "cpu")),
            pretrained=pretrained,
            # cache_dir=cache_dir   # YOUR local cache path
        )
    ...
    @staticmethod
    def _resolve_local_open_clip_checkpoint(arch: str, version: str) -> str:
        override = os.environ.get("OPENCLIP_CKPT", "").strip()
        ...
        if arch == "ViT-H-14" and version == "laion2b_s32b_b79k":
            ...
            repo_dir = "models--laion--CLIP-ViT-H-14-laion2B-s32B-b79K"
            snapshot = Path(HF_HUB_CACHE) / repo_dir / "snapshots" / OPENCLIP_H_REVISION
            checkpoint = snapshot / "open_clip_model.safetensors"
            if checkpoint.is_file():
                return str(checkpoint)
            raise FileNotFoundError(
                f"Pinned OpenCLIP safetensors checkpoint not found: {checkpoint}. "
                "Run `python scripts/download_ckpt.py --name motionpro`."
            )
        return version
