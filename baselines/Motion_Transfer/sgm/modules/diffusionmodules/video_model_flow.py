...
from torch.utils.checkpoint import checkpoint # gradient checkpointing from pytorch
...
from ...modules.video_attention import SpatialVideoTransformer    #  the same
from ...modules.attention import SpatialTransformer     #  the same
from ...util import default
from .util import (AlphaBlender, avg_pool_nd, conv_nd, linear,   # the same
                                              normalization,
...
# ! NOTE: only change the checkpoint from pytorch
    ...
    def forward(
        ...
        for layer in self:
            ...
            if isinstance(module, TimestepBlock) and not isinstance(module, VideoResBlock) and not isinstance(module, VideoResBlock_Embed):
                ...
                x = layer(x, emb, num_video_frames, image_only_indicator, flow)     # *zzw: inject here
...
class ResBlock(TimestepBlock):   # the same..
    """
    ...
    def __init__(
        ...
        if up:
            self.h_upd = Upsample(channels, False, dims)    # the same..
            self.x_upd = Upsample(channels, False, dims)    # the same..
        elif down:
...
class VideoResBlock(ResBlock):    # the same..
    def __init__(
        self,
        channels: int,
        emb_channels: int,
...
class VideoUNet_flow(nn.Module):
    def __init__(
        ...
        def get_resblock(
            ...
            return VideoResBlock(               # all the same beside checkpoint
                merge_factor=merge_factor,
                merge_strategy=merge_strategy,
        ...
        for level, mult in enumerate(channel_mult):
            for i in range(num_res_blocks):
                if i == 0:
                    layers = [
                        get_embed_resblock(                     # only at first block inject..
                            merge_factor=merge_factor,
                            merge_strategy=merge_strategy,
                            video_kernel_size=video_kernel_size,
                            ch=ch,
    ...
    def forward(
        ...
        for module in self.flow_blocks:
            flow = module(flow, emb=None, num_video_frames=num_video_frames)        #[f,c,320,576]-->[320,40,72]-->[640,20,36]-->[1280,10,18]-->[1280,5,9]
