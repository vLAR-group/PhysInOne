...
class WanImageToVideoPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    ...
    def __call__(
        ...
        Args:
            ...
            fps (`int`, *optional*, defaults to 7):
                Frames per second. This is useless for Wan2.2
    ...
    def training(
        ...
        generator: Optional[Union[torch.Generator, List[torch.Generator]]] = None,
        max_sequence_length: int = 512,
        conditioning_dropout_prob: float = 0.2,
        fps: int = 24,
    ):
        r"""
        The call function to the pipeline for generation.
        ...
        with torch.no_grad():
            ...
            frame_mask = adjust_batch_masks_to_n_plus_1(frame_mask, k=4)
            attention_kwargs = {
                "attention_mask": frame_mask
            }
            self._attention_kwargs = attention_kwargs
            dtype = self.transformer.dtype if self.transformer is not None else self.transformer_2.dtype

            # Preprocess prompt to prompt embedding
            prompt_embeds, negative_prompt_embeds = self.encode_prompt(
                prompt=prompt,
                negative_prompt=negative_prompt,
                do_classifier_free_guidance=False, # avoiding generate negative prompt embedding
                max_sequence_length=max_sequence_length,
                device=device,
            )

            # Preprocess image to image embedding TODO: Check if this is at the first frame of the noise latent
            # image_embeds = self.encode_image(image, device)
            ...
            video_latents = self.vae.encode(video, return_dict=False)[0].sample()
            latents_mean = (
                torch.tensor(self.vae.config.latents_mean)
                .view(1, self.vae.config.z_dim, 1, 1, 1)
            ...
            ) # inverse to the one in __call__
            video_latents = (video_latents - latents_mean) / latents_std + latents_mean
            #  Wan2.2 there is no scale factor in VAE
            ...
            if self.config.expand_timesteps:
                # wan 2.2 5b i2v use firt_frame_mask to mask timesteps
                noise, condition, first_frame_mask = latents_outputs
            else:
                noise, condition = latents_outputs
            ...
            if conditioning_dropout_prob > 0.0:
                ...
                null_conditioning = torch.zeros_like(prompt_embeds).detach()
                prompt_embeds = torch.where(prompt_mask, null_conditioning, prompt_embeds)

        # denoise
        denoiser = self.transformer
        # TODO: Write by me may be wrong
        training_target = self.scheduler.training_target(noisy_latents, video_latents, noise, timesteps)
        # UniPC: A Unified Predictor-Corrector Framework for Fast Sampling of Diffusion Models
        # prediction type : flow matching and predict x0
        ...
        # TODO: Average on all or only valid frame?
