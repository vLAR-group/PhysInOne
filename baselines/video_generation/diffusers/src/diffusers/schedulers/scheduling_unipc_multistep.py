...
class UniPCMultistepScheduler(SchedulerMixin, ConfigMixin):
    ...
    def training_target(self, sample, original_samples, noise, timesteps):
        # Gather the corresponding sigma values for all timesteps in the batch
        if self.config.prediction_type == "epsilon":
            target = noise
            # x0_pred = (sample - sigma_t * model_output) / alpha_t
        elif self.config.prediction_type == "sample":
            ...
            # Ensure x0_pred is computed correctly if needed
            timesteps = timesteps.cpu()
            sigma = self.sigmas[timesteps]  # Assuming self.sigmas is a tensor that supports indexing
            alpha_t, sigma_t = self._sigma_to_alpha_sigma_t(sigma)
            alpha_t = alpha_t.reshape(alpha_t.size()[0], 1, 1, 1, 1).to(original_samples.device)
            sigma_t = sigma_t.reshape(alpha_t.size()[0], 1, 1, 1, 1).to(original_samples.device)
            target = (alpha_t * sample - original_samples) / sigma_t
            # x0_pred = alpha_t * sample - sigma_t * model_output
        elif self.config.prediction_type == "flow_prediction":
            target = noise - original_samples

        return target  # Return the tensor of targets for the batch
