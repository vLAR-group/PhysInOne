...
from ..pipeline_utils import DiffusionPipeline, adjust_batch_masks_to_n_plus_1
    ...
    def training(
            ...
            Args:
                image (`PipelineImageInput`):
                    The input image to condition the generation on. Must be a `torch.Tensor`. B C H W
                video (`torch.Tensor`):
                    The ground truth. Must be a `torch.Tensor`. B F C H W
                prompt (`str` or `List[str]`, *optional*):
                    prompt to be encoded
                negative_prompt (`str` or `List[str]`, *optional*):
                    The prompt or prompts not to guide the image generation. If not defined, one has to pass
            ...
            with torch.no_grad():
                ...
                } # But it seems no place to input
                ...
                vae_dtype = self.vae.dtype
                # Now the transformer actually a LoraModel
                num_channels_latents = self.transformer.model.config.in_channels - 1

                # prepare video latent (gt)
                video = video.permute(0, 2, 1, 3, 4) # from BCFHW to BFCHW
                video = self.video_processor.preprocess_video(video, height, width)
                video = video.to(device=device, dtype=vae_dtype)
                _, video_latents, _, _, _, _ = self.prepare_latents(
                    video,
                    batch_size * num_videos_per_prompt,
                    num_channels_latents,
                    height,
                ...
                conditioning = self.video_processor.preprocess_video(image, height, width)
                conditioning = conditioning.to(device=device, dtype=vae_dtype)
                noise, conditioning_latents, cond_indicator, _, cond_mask, _ = self.prepare_latents(
                    conditioning,
                    batch_size * num_videos_per_prompt,
                    num_channels_latents,
                    height,
                ...
                # TODO: Does it need descaling for noise and video latents?
                noise = noise / self.scheduler.config.sigma_max

                # unconditioning_latents = None
                cond_mask = cond_mask.to(dtype)
                padding_mask = noise.new_zeros(1, 1, height, width, dtype=dtype)
                sigma_conditioning = torch.tensor(sigma_conditioning, dtype=torch.float32, device=device)
                t_conditioning = sigma_conditioning / (sigma_conditioning + 1)
                
                num_train_timesteps = 1000
                self.scheduler.set_timesteps(num_train_timesteps, device=device)
                timesteps = torch.randint(0, self.scheduler.num_train_timesteps, (batch_size,))
                timesteps = timesteps.long().to(device)

                current_sigma = self.scheduler.sigmas[timesteps]

                current_t = current_sigma / (current_sigma + 1)
                current_t = current_t.view(batch_size, 1, 1, 1, 1)
                c_in = 1 - current_t
                c_skip = 1 - current_t
                c_out = -current_t
                cond_timestep = current_t.expand(
                    -1, -1, noise.size(2), -1, -1
                )  # [B, 1, T, 1, 1]

                noisy_latent = self.scheduler.scale_noise(sample=video_latents, noise=noise, timestep=current_sigma*num_train_timesteps)
                cond_latent = noisy_latent * c_in
                cond_latent = cond_indicator * conditioning_latents + (1 - cond_indicator) * cond_latent
                cond_latent = cond_latent.to(dtype)
                cond_timestep = cond_indicator * t_conditioning + (1 - cond_indicator) * cond_timestep
                cond_timestep = cond_timestep.to(dtype)
                

            # denoise
            denoiser = self.transformer
            # target: flow matching
            # self.config.stochastic_sampling = False
            # prev_sample = sample + dt * model_output
            # dt = sigma_next - sigma
            training_target = video_latents - noise
            ...
            model_output = (c_skip * noisy_latent + c_out * model_output.float()).to(dtype)
            model_output = cond_indicator * conditioning_latents + (1 - cond_indicator) * model_output
            # model_output = (noisy_latent - model_output) 
            model_output = (noisy_latent - model_output) / current_sigma.view(batch_size, 1, 1, 1, 1)

            _, C, _, H, W = model_output.size()
            frame_mask = frame_mask.repeat(1, C, 1, H, W)
            masked_diff = (model_output.float()-training_target.float())[frame_mask == 1.0]
            loss = torch.nn.functional.mse_loss(masked_diff, torch.zeros_like(masked_diff))
