...
def adjust_batch_masks_to_n_plus_1(batch_masks, k: int = 4):
    """
    Adjust a batch of masks by converting minimal 1s to 0s so that each mask
    has a number of 1s in the form 4n+1.
    
    Parameters:
        batch_masks: Tensor of shape [batch_size, seq_len] where each mask
                     consists of 1s followed by 0s
        
    Returns:
        adjusted_batch: Modified batch of masks with shape [batch_size, seq_len]
        removed_counts: Tensor of shape [batch_size] indicating number of 1s removed per mask
    """
    batch_size, _, seq_len, _, _ = batch_masks.shape
    adjusted_batch = torch.zeros((batch_size, seq_len // k + 1), dtype=torch.float16)
    
    # Calculate total ones for each mask in the batch
    total_ones = torch.sum(batch_masks, dim=2).long()  # Shape: [batch_size, 1, 1, 1]
    n = total_ones // k + 1
    
    # Process each mask in the batch
    for i in range(batch_size):
        # Get target for this mask
        target = n[i, 0, 0, 0].item()
        # Convert excess 1s to 0s (starting from the rightmost 1)
        if target < seq_len:
            adjusted_batch[i, :int(target)] = 1.0
    adjusted_batch = adjusted_batch.reshape(adjusted_batch.shape[0], 1, adjusted_batch.shape[1], 1, 1)
